...
f_L_z <- function(selected_interval, Epsilon, z, a, b, lambda2, lambda3){
  ...
  result <- ifelse(selected_interval == 1, log(1+z),
                   ifelse(selected_interval == 2, f_e_x(Epsilon, a/Epsilon) + lambda2 * (z - a/Epsilon),
                          ifelse(selected_interval == 3, f_e_x(Epsilon, 1/Epsilon) + lambda3 * (z - b/Epsilon), f_e_x(Epsilon, b/Epsilon) + Epsilon * (z - b/Epsilon))))
...
sample_z <- function(selected_interval, Epsilon,
  ...
  z <- ifelse(selected_interval == 1, (1+a/Epsilon)^u - 1,
              ifelse(selected_interval == 2, a/Epsilon - log(1 - u + u * exp(-(f_e_x(Epsilon, 1/Epsilon) - f_e_x(Epsilon, a/Epsilon))))/lambda2,
                     ifelse(selected_interval == 3, 1/Epsilon - log(1 - u + u * exp(-(f_e_x(Epsilon, b/Epsilon) - f_e_x(Epsilon, 1/Epsilon))))/lambda3, b/Epsilon - log(1 - u)/Epsilon)))
